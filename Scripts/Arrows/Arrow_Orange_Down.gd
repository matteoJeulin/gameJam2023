...
var type = Types.orange.new()
var direction = 0
var directions = ["down", "right","up", "left"]

func _ready() :
	get_node("../Switch_Arrow_Orange_Antitrigo").connect("switch_" + type.color + "_antitrigo", switch_antitrigo)
	get_node("../Switch_Arrow_Orange_Trigo").connect("switch_" + type.color + "_trigo", switch_trigo)
